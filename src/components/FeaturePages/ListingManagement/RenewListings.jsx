...
export default function RenewListings({ feature }) {
  const selectedAccountIds = useAppStore((s) => s.selectedAccountIds)
  const [listingIds, setListingIds] = useState('')
  const [maxRenew, setMaxRenew] = useState(10)
  ...
  const handleAction = async () => {
    ...
    await runTask('renew-listings', {
      ...
      max_renew: Math.min(200, Math.max(1, maxRenew)),
  ...
  return (
    <PageShell title={feature.title} description={feature.description} actions={
      ...
      <div className="grid lg:grid-cols-[320px_1fr] gap-5">
        ...
        <div className="space-y-4">
          <ConfigPanel title="Renew Settings" icon="🔄">
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <Field label="Listing IDs (comma separated)" hint="Leave empty to renew all active listings">
                  <input className="input" value={listingIds} onChange={(e) => setListingIds(e.target.value)} placeholder="Leave empty to renew all" />
                </Field>
              </div>
              <Field label="Max renew count (1–200)">
                <input className="input" type="number" min={1} max={200} value={maxRenew} onChange={(e) => setMaxRenew(Number(e.target.value))} />
              </Field>
              <Field label="Delay between renewals (sec)">
                <input className="input" type="number" min={2} max={120} value={delaySeconds} onChange={(e) => setDelaySeconds(Number(e.target.value))} />
              </Field>
            </div>
          </ConfigPanel>
          <ListingsTable title="Active Listings" />
        </div>
      </div>
    </PageShell>
  )
