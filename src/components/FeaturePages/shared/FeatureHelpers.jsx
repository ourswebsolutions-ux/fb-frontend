...
export function useAutomationTask() {
  ...
  const runTask = useCallback(async (route, payload = {}) => {
    setBusy(true); setStatusMessage(''); setTask(null); setTaskId(null)
    try {
      const data = await api.startAutomation(route, payload)
      if (data.task_id) { setTaskId(data.task_id); setStatusMessage(data.message || `Task started`) }
      else setStatusMessage(data.message || JSON.stringify(data))
      return data
    } catch (err) {
  ...
  const reset = useCallback(() => {
    setStatusMessage(''); setBusy(false); setTaskId(null); setTask(null); setPolling(false)
  }, [])
